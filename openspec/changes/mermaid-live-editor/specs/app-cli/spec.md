...
The system SHALL be invocable from the command line using `npx mermaid-live <file>` or directly via the installed binary. The file argument SHALL be a path to a `.mmd` file. If the file does not exist, it SHALL be created with default template content.

#### Scenario: Open existing file
- **WHEN** user runs `npx mermaid-live architecture.mmd` and the file exists
...
- **WHEN** user runs `npx mermaid-live new_diagram.mmd` and the file does not exist
...
- **WHEN** user runs `npx mermaid-live` without specifying a file
...
- **THEN** the window title displays "System Architecture — Mermaid Live"

#### Scenario: File without frontmatter title
- **WHEN** the file has no frontmatter title
- **THEN** the window title displays the filename (e.g., "architecture.mmd — Mermaid Live")
